...
function formatTweetText(text) {
    return text
        ...
        .replace(/(https?:\/\/[^\s]+)/g, '<a href="$1" target="_blank" style="color:#a78bfa">$1</a>')
        .replace(/\n/g, '<br>');
}
...
function copyLink() {
    navigator.clipboard.writeText(window.location.href).then(() => {
        ...
        toast.style.cssText = 'position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:#7c3aed;color:#fff;padding:12px 24px;border-radius:12px;font-size:14px;font-weight:600;z-index:999;animation:fadeInUp 0.3s;box-shadow:0 8px 32px rgba(124,58,237,0.4)';
